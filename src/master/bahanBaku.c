...
int hitungBahanAktif()
{
    ...
    for (int i = 0; i < totalBahan; i++)
    {
        if (daftarBahan[i].status == 1) // asumsi ada field status
            count++;
    }
    return count;
}
...
void ubahBahan()
{
    ...
    while (1)
    {
        ...
        if (idx == -1)
        {
            popupAlert(0, "No Tidak Ditemukan!");
            continue;
        }
        ...
        frame(left, top, right, bot);
        gotoxy(left + 20, top - 2);
        printf("\033[41m\033[93m UBAH DATA BAHAN \033[0m");
        setRGBColor(202, 40, 44, 1);
        setRGBColor(251, 255, 199, 0);
        gotoxy(left, top - 1);
